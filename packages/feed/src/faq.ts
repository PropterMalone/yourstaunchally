...
<p>Phases advance at the deadline, or early if all players submit orders AND signal <code>ready</code> (with a 30-minute grace period to revise). Submitting new orders clears your ready status. All deadlines are in <strong>UTC</strong>.</p>
...
<ul>
  ...
  <li><code>#id possible F NTH</code> — See options for a specific unit</li>
  <li><code>#id orders</code> — Review your submitted orders</li>
  <li><code>#id cancel</code> — Cancel all submitted orders</li>
  <li><code>#id cancel A PAR</code> — Cancel a single unit's order</li>
  <li><code>#id ready</code> — Signal ready for early adjudication</li>
  <li><code>#id unready</code> — Withdraw ready signal</li>
  <li><code>#id draw</code> — Vote for a draw (private, via DM)</li>
...
<table>
  ...
  <tr><td>Build</td><td><code>UNIT LOC B</code> or just <code>UNIT LOC</code></td><td><code>F LON B</code> or <code>F LON</code></td></tr>
...
<p><strong>Relaxed syntax:</strong> Hyphens don't need spaces (<code>F BRE-MAO</code> works). In supports and convoys, the unit type can be omitted for armies (<code>A MAR S PAR - BUR</code> instead of <code>A MAR S A PAR - BUR</code>). During retreats, plain moves work (<code>A BUR - PAR</code> instead of <code>A BUR R PAR</code>). During builds, bare units are accepted (<code>F LON</code> instead of <code>F LON B</code>).</p>
