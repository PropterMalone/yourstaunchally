...
export function normalizeOrderString(order: string): string {
	...
	// Infer missing unit type after S or C: "A MAR S PAR - BUR" → "A MAR S A PAR - BUR"
	// Convoys always convoy armies; for supports, default to army.
	normalized = normalized
		.replace(/^([AF] [A-Z]{3}(?:\/[NSEW]C)? [SC]) ([A-Z]{3}(?:\/[NSEW]C)?)([ -])/, '$1 A $2$3')
		// Handle support-hold without move (no trailing dash): "A MAR S PAR" at end of string
		.replace(/^([AF] [A-Z]{3}(?:\/[NSEW]C)? [SC]) ([A-Z]{3}(?:\/[NSEW]C)?)$/, '$1 A $2');
...
/**
 * During adjustment phases, convert bare unit specifications ("F EDI", "A MUN")
 * into build orders by appending " B". Only applies when the phase ends in 'A'
 * and the order is exactly a unit type + province with no action suffix.
 */
export function inferBuildOrders(orders: string[], phase: string): string[] {
	if (!phase.endsWith('A')) return orders;
	return orders.map((o) => {
		const upper = o.trim().toUpperCase();
		if (/^[AF] [A-Z]{3}$/.test(upper)) return `${upper} B`;
		return o;
	});
}
