...
/** Mark a power as ready for immediate adjudication */
export function setReady(
	...
	if (state.readyPowers.includes(power)) {
		return { ok: false, error: `${power} is already ready` };
	}
	return {
		ok: true,
		state: { ...state, readyPowers: [...state.readyPowers, power] },
	};
}

/** Unmark a power as ready */
export function unsetReady(
	...
	if (!state.readyPowers.includes(power)) {
		return { ok: false, error: `${power} is not marked as ready` };
	}
	return {
		ok: true,
		state: { ...state, readyPowers: state.readyPowers.filter((p) => p !== power) },
	};
}

/** Check if all powers with orders are also ready for immediate adjudication */
export function allReadyForAdjudication(state: GameState): boolean {
	if (!allOrdersSubmitted(state)) return false;
	const pendingReady = getPendingPowers(state); // should be empty if all orders in
	if (pendingReady.length > 0) return false;

	// All powers that must order need to be ready
	const assignedPowers = state.players.map((p) => p.power).filter((p): p is Power => p !== null);
	const mustOrder = state.orderablePowers
		? assignedPowers.filter((p) => (state.orderablePowers as Power[]).includes(p))
		: assignedPowers;
	return mustOrder.every((p) => state.readyPowers.includes(p));
}
