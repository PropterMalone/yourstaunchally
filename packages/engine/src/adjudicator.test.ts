...
describe('adjudicator integration', () => {
	...
	it('returns orders SVG with move arrows when render is enabled', async () => {
		const game = await newGame();

		const result = await setOrdersAndProcess(
			game.gameState,
			{
				FRANCE: ['A PAR - BUR', 'A MAR - SPA', 'F BRE - MAO'],
			},
		...
		expect(result.ordersSvg).toBeDefined();
		expect(result.ordersSvg).toContain('<?xml');
		// Orders SVG should contain order layer elements
		expect(result.ordersSvg).toContain('svg');
	});
