...
export type DmCommand =
	...
	| { type: 'show_possible'; gameId: string; unitFilter?: string }
...
export function parseDm(text: string): DmCommand {
	...
	if (lower === 'ready' || lower === 'confirm' || lower === 'rdy') {
		return { type: 'queue_ready' };
	}
	...
	if (
		lowerRest === 'possible' ||
		lowerRest.startsWith('possible ') ||
		lowerRest === 'options' ||
		lowerRest === 'show possible' ||
		lowerRest === 'help'
	) {
		// Check for per-unit filter: "possible F NTH" or "possible A PAR"
		const unitFilterMatch = rest.match(
			/^(?:possible|options)\s+([AF])\s+([A-Z]{3}(?:\/[NSEW]C)?)/i,
		);
		if (unitFilterMatch) {
			const unitFilter = `${(unitFilterMatch[1] as string).toUpperCase()} ${(unitFilterMatch[2] as string).toUpperCase()}`;
			return { type: 'show_possible', gameId, unitFilter };
		}
	...
	if (lowerRest === 'ready' || lowerRest === 'rdy') {
		return { type: 'ready', gameId };
	}

	if (lowerRest === 'unready' || lowerRest === 'not ready' || lowerRest === 'wait') {
		return { type: 'unready', gameId };
	}
