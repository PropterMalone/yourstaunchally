...
export function createLlmClient(ollamaUrl?: string): LlmClient {
	...
	async function generateResponse(context: LlmContext): Promise<string | null> {
		...
		const sanitizedContext = context.playerMessage
			? { ...context, playerMessage: sanitizeInput(context.playerMessage) }
			: context;

		const userMsg = buildUserMessage(sanitizedContext);
		...
		try {
			...
			const res = await fetch(`${baseUrl}/api/generate`, {
				...
				body: JSON.stringify({
					model: MODEL,
					prompt: userMsg,
					system: getSystemPrompt(sanitizedContext.power),
					stream: false,
					options: {
						temperature: 0.7,
