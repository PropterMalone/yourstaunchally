...
describe('parseDm', () => {
	...
	it('parses bare "ready" DM as queue_ready', () => {
		expect(parseDm('ready')).toEqual({ type: 'queue_ready' });
	});

	it('parses "confirm" DM as queue_ready', () => {
		expect(parseDm('confirm')).toEqual({ type: 'queue_ready' });
	});

	it('parses "rdy" DM as queue_ready', () => {
		expect(parseDm('rdy')).toEqual({ type: 'queue_ready' });
	});

	it('parses "#gameId ready" as game ready (not queue_ready)', () => {
		expect(parseDm('#abc123 ready')).toEqual({ type: 'ready', gameId: 'abc123' });
	});
