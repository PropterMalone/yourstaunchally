...
export interface GameDb {
	...
	/** Mark a queued player as confirmed for ready check */
	confirmPlayer(did: string): void;
	/** Reset all confirmations (e.g. when ready check expires) */
	resetConfirmations(): void;
	/** Get queued players who haven't confirmed */
	getUnconfirmedPlayers(): { did: string; handle: string }[];
	/** Check if all queued players (up to maxPlayers) are confirmed */
	allConfirmed(maxPlayers: number): boolean;
...
export function createDb(config: DbConfig): GameDb {
	...
	return {
		init() {
			...
			// Migration: add confirmed column to signup_queue
			const queueCols = db.pragma('table_info(signup_queue)') as { name: string }[];
			if (!queueCols.some((c) => c.name === 'confirmed')) {
				db.exec('ALTER TABLE signup_queue ADD COLUMN confirmed INTEGER NOT NULL DEFAULT 0');
			}
		},
		...
		confirmPlayer(did: string) {
			db.prepare('UPDATE signup_queue SET confirmed = 1 WHERE did = ?').run(did);
		},

		resetConfirmations() {
			db.prepare('UPDATE signup_queue SET confirmed = 0').run();
		},

		getUnconfirmedPlayers(): { did: string; handle: string }[] {
			return db
				.prepare('SELECT did, handle FROM signup_queue WHERE confirmed = 0 ORDER BY queued_at ASC')
				.all() as { did: string; handle: string }[];
		},

		allConfirmed(maxPlayers: number): boolean {
			const queue = db
				.prepare('SELECT confirmed FROM signup_queue ORDER BY queued_at ASC LIMIT ?')
				.all(maxPlayers) as { confirmed: number }[];
			return queue.length >= maxPlayers && queue.every((r) => r.confirmed === 1);
		},
