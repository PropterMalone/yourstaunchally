...
export function createGameManager(deps: GameManagerDeps) {
	...
	/** Hours before a ready check expires */
	const READY_CHECK_HOURS = 24;

	async function matchmakeFromQueue(_reply: (text: string) => Promise<void>): Promise<void> {
		const queue = db.getQueue();
		if (queue.length < config.maxPlayers) return;

		// If a ready check is already active, don't start another
		const existingDeadline = db.getBotState('ready_check_deadline');
		if (existingDeadline) return;

		await startReadyCheck();
	}

	async function startReadyCheck(): Promise<void> {
		const deadline = new Date(Date.now() + READY_CHECK_HOURS * 60 * 60 * 1000).toISOString();
		db.setBotState('ready_check_deadline', deadline);
		db.resetConfirmations();
		...
		console.log(
			`[queue] Ready check started for ${config.maxPlayers} players, deadline ${deadline}`,
		);
		...
		for (const player of players) {
			try {
				await dmSender.sendDm(
					player.did,
					`Queue is full! (${handles})\n\nReply "ready" to confirm you're still in. You have ${READY_CHECK_HOURS}h.\n\nIf you're no longer interested, reply "leave queue".`,
				);
			} catch (error) {
				console.warn(`[queue] Failed to DM ready check to ${player.handle}: ${error}`);
			}
		}
	}

	async function completeMatchmaking(reply: (text: string) => Promise<void>): Promise<void> {
		const queue = db.getQueue();
		const players = queue.slice(0, config.maxPlayers);
		const gameId = generateGameId();
		...
		db.setBotState('ready_check_deadline', '');
		db.saveGame(state);

		const handles = players.map((p) => `@${p.handle}`).join(', ');
		await reply(`All confirmed! Starting game #${gameId} with ${handles}`);

		await doStartGame(state, reply);
	}

	async function expireReadyCheck(): Promise<void> {
		const unconfirmed = db.getUnconfirmedPlayers();
		if (unconfirmed.length === 0) return;

		console.log(`[queue] Ready check expired, removing ${unconfirmed.length} unconfirmed players`);

		for (const player of unconfirmed) {
			db.dequeuePlayer(player.did);
			try {
				await dmSender.sendDm(
					player.did,
					'You didn\'t confirm in time and have been removed from the queue. DM "play" to rejoin.',
				);
			} catch (error) {
				console.warn(`[queue] Failed to DM expiry to ${player.handle}: ${error}`);
			}
		}
		...
		// Notify remaining confirmed players
		const remaining = db.getQueue();
		for (const player of remaining) {
			try {
				await dmSender.sendDm(
					player.did,
					`Some players didn't confirm. You're still in the queue (${remaining.length}/${config.maxPlayers}). Waiting for more players.`,
				);
			} catch (error) {
				console.warn(`[queue] Failed to DM remaining to ${player.handle}: ${error}`);
			}
		}
	}

	async function handleQueueReadyDm(dm: InboundDm): Promise<void> {
		if (!db.isQueued(dm.senderDid)) {
			await dmSender.sendDm(dm.senderDid, "You're not in the queue.");
			return;
		}

		const deadline = db.getBotState('ready_check_deadline');
		if (!deadline) {
			await dmSender.sendDm(
				dm.senderDid,
				"No ready check active right now. You're in the queue — game starts when we have enough players.",
			);
			return;
		}

		db.confirmPlayer(dm.senderDid);

		if (db.allConfirmed(config.maxPlayers)) {
			console.log('[queue] All players confirmed, starting game');
			const noopReply = async (text: string) => {
				console.log(`[queue] matchmake: ${text}`);
			};
			await completeMatchmaking(noopReply);
		} else {
			const unconfirmed = db.getUnconfirmedPlayers();
			const confirmedCount = config.maxPlayers - unconfirmed.length;
			await dmSender.sendDm(
				dm.senderDid,
				`Confirmed! (${confirmedCount}/${config.maxPlayers}) Waiting on ${unconfirmed.length} more.`,
			);
		}
	}

	async function cancelReadyCheckIfNeeded(): Promise<void> {
		const deadline = db.getBotState('ready_check_deadline');
		if (!deadline) return;

		const size = db.getQueueSize();
		if (size >= config.maxPlayers) return;

		console.log(`[queue] Ready check cancelled (${size}/${config.maxPlayers} players)`);
		db.setBotState('ready_check_deadline', '');
		db.resetConfirmations();
		...
		for (const player of remaining) {
			try {
				await dmSender.sendDm(
					player.did,
					`A player left the queue, so the ready check has been cancelled. You're still in the queue (${size}/${config.maxPlayers}). Waiting for more players.`,
				);
			} catch (error) {
				console.warn(`[queue] Failed to DM cancel to ${player.handle}: ${error}`);
			}
		}
	}
	...
	async function handleLeaveQueue(
		...
		await cancelReadyCheckIfNeeded();
	}
	...
	async function handlePlayDm(dm: InboundDm): Promise<void> {
		...
		if (db.isQueued(dm.senderDid)) {
			...
			await dmSender.sendDm(
				dm.senderDid,
				`You're already in the queue (${size}/${config.maxPlayers}).`,
			);
			return;
		}
	...
	async function handleLeaveQueueDm(dm: InboundDm): Promise<void> {
		...
		await dmSender.sendDm(
			dm.senderDid,
			`You've left the queue. (${size}/${config.maxPlayers} remaining)`,
		);

		// If a ready check was active, cancel it since we're now below maxPlayers
		await cancelReadyCheckIfNeeded();
	}
	...
	async function handleDm(dm: InboundDm): Promise<void> {
		...
		switch (command.type) {
			...
			case 'cancel_order':
				...
				break;
			case 'draw':
				await handleDrawDm(command, dm);
				break;
			case 'ready':
				await handleReadyDm(command, dm);
				break;
			case 'unready':
				await handleUnreadyDm(command, dm);
				break;
			case 'my_games':
	...
	async function handleOrderSubmission(
		...
		const afterBuilds = state.currentPhase
			? inferBuildOrders(afterNorm, state.currentPhase)
			: afterNorm;
		const normalized = state.currentPhase
			? convertMovesToRetreats(afterBuilds, state.currentPhase)
			: afterBuilds;
		...
		let result = submitOrders(state, power, orders);
		if (!result.ok) {
		...
		if (state.diplomacyState) {
			try {
				...
				const orderableLocs = powerOrders
					? new Set(Object.keys(powerOrders).map((l) => l.toUpperCase()))
					: new Set<string>();

				// Discard orders for nonexistent units (e.g. F BOT when no fleet in Bothnia).
				// Keep invalid orders for real units (wrong destination) as warnings.
				const phantomUnits: string[] = [];
				const validatedOrders = allOrders.filter((order) => {
					if (order === 'WAIVE') return true;
					const unitLoc = order.split(/\s+/).slice(0, 2).join(' ');
					const province = unitLoc.split(' ')[1] ?? '';
					if (orderableLocs.size > 0 && !orderableLocs.has(province)) {
						phantomUnits.push(order);
						return false;
					}
					return true;
				});

				// If we filtered out phantom orders, update saved state
				if (phantomUnits.length > 0) {
					const cleanedOrders = { ...result.state.currentOrders };
					const existing = cleanedOrders[power];
					if (existing) {
						const phantomSet = new Set(phantomUnits.map((o) => o.toUpperCase()));
						cleanedOrders[power] = {
							...existing,
							orders: existing.orders.filter((o) => !phantomSet.has(o.toUpperCase())),
						};
						const cleanedState = { ...result.state, currentOrders: cleanedOrders };
						db.saveGame(cleanedState);
						result = { ...result, state: cleanedState };
					}
				}
				...
				let illegalWarning = '';
				if (illegalCount > 0) {
					illegalWarning = `\n\n${illegalOrderCommentary(illegalCount, validatedOrders.length)}`;
				}

				// Notify about discarded phantom orders
				let phantomWarning = '';
				if (phantomUnits.length > 0) {
					phantomWarning = `\n\nDiscarded (no unit there): ${phantomUnits.join(', ')}`;
				}

				const orderSummary = annotated.join('\n');
				await dmSender.sendDm(
					dm.senderDid,
					`${power} orders for #${command.gameId} (${validatedOrders.length}):\n${orderSummary}${illegalWarning}${phantomWarning}\n\nSend orders for specific units to update them.`,
				);
			} catch (error) {
		...
		// Submitting new orders clears ready status (player may want more negotiation time)
		if (result.state.readyPowers.includes(power)) {
			result = {
				...result,
				state: {
					...result.state,
					readyPowers: result.state.readyPowers.filter((p) => p !== power),
				},
			};
			db.saveGame(result.state);
		}

		// Check if all orders AND all ready → trigger grace period
		await maybeStartGracePeriod(result.state, command.gameId, dm.senderDid);
	}

	/** Start grace period if all orders are in AND all players are ready */
	async function maybeStartGracePeriod(
		state: GameState,
		gameId: string,
		triggerDid?: string,
	): Promise<void> {
		if (!allReadyForAdjudication(state)) return;

		const GRACE_PERIOD_MS = 30 * 60 * 1000;
		const graceDeadline = new Date(Date.now() + GRACE_PERIOD_MS).toISOString();
		const currentDeadline = state.phaseDeadline;

		// Only shorten — never extend past the original deadline
		if (currentDeadline && new Date(graceDeadline) >= new Date(currentDeadline)) return;

		const updated = { ...state, phaseDeadline: graceDeadline };
		db.saveGame(updated);
		console.log(`[orders] All orders+ready for #${gameId}, grace period until ${graceDeadline}`);

		// Public announcement
		const graceMsg = `⏰ Game #${gameId} — all orders are in! ${allOrdersInCommentary()}\n\nAdjudication in 30 minutes. You may revise orders until then.`;
		const prev = db.getLatestMapPost(gameId);
		...
		for (const player of state.players) {
			if (!player.did) continue;
			const msg =
				player.did === triggerDid
					? `🚨 Grace period started for #${gameId} — adjudication in 30 minutes!\n\nAll players can still revise orders until then.`
					: `⏰ All orders are in for #${gameId}. Adjudication in 30 minutes — DM revised orders now if you want to change anything.`;
			try {
	...
	async function handleReadyDm(
		command: DmCommand & { type: 'ready' },
		dm: InboundDm,
	): Promise<void> {
		const state = db.loadGame(command.gameId);
		if (!state) {
		...
		const result = setReady(state, power);
		if (!result.ok) {
			await dmSender.sendDm(dm.senderDid, result.error);
			return;
		}
		...
		db.saveGame(result.state);
		const pending = getPendingPowers(result.state);
		const readyCount = result.state.readyPowers.length;
		const assignedPowers = result.state.players
			.map((p) => p.power)
			.filter((p): p is Power => p !== null);
		const mustOrder = result.state.orderablePowers
			? assignedPowers.filter((p) => (result.state.orderablePowers as Power[]).includes(p))
			: assignedPowers;

		await dmSender.sendDm(
			dm.senderDid,
			`${power} is ready for adjudication in #${command.gameId}. (${readyCount}/${mustOrder.length} ready, ${pending.length} still need orders)\n\nWhen all players have orders in and are ready, adjudication starts with a 30-min grace period.\nDM "#${command.gameId} unready" to change your mind.`,
		);

		await maybeStartGracePeriod(result.state, command.gameId, dm.senderDid);
	}

	async function handleUnreadyDm(
		command: DmCommand & { type: 'unready' },
		dm: InboundDm,
	): Promise<void> {
		const state = db.loadGame(command.gameId);
		if (!state) {
		...
		const result = unsetReady(state, power);
		if (!result.ok) {
			await dmSender.sendDm(dm.senderDid, result.error);
			return;
		}
		...
		await dmSender.sendDm(
			dm.senderDid,
			`${power} is no longer ready for immediate adjudication in #${command.gameId}. Phase will run until the deadline.`,
		);
	}
	...
	async function handleDrawDm(command: DmCommand & { type: 'draw' }, dm: InboundDm): Promise<void> {
		const state = db.loadGame(command.gameId);
		if (!state) {
			await dmSender.sendDm(dm.senderDid, `Game #${command.gameId} not found.`);
			return;
		...
		if (result.drawAchieved) {
			...
			await dmSender.sendDm(dm.senderDid, `Draw achieved! Game #${command.gameId} is over.`);
		} else {
			const total = state.players.filter((p) => p.power).length;
			await dmSender.sendDm(
				dm.senderDid,
				`${power} votes for a draw in #${command.gameId}. (${result.state.drawVote.votedPowers.length}/${total} needed)`,
			);
		}
	}
	...
	async function handleShowPossible(
		...
		// Per-unit filter: show all options for a specific unit
		if (command.unitFilter) {
			const filterProvince = command.unitFilter.split(' ')[1] ?? '';
			const matchingLoc = Object.keys(powerOrders).find(
				(loc) => loc.toUpperCase() === filterProvince,
			);
			if (!matchingLoc) {
				await dmSender.sendDm(
					dm.senderDid,
					`No unit at ${filterProvince} for ${power}. Your units: ${Object.keys(powerOrders).join(', ')}`,
				);
				return;
			}
			const unitOrders = (powerOrders[matchingLoc] ?? []).map((o) =>
				o.replace(/ VIA$/, ' (via convoy)'),
			);
			let msg = `${command.unitFilter} — ${unitOrders.length} options:\n\n${unitOrders.join('\n')}`;
			if (msg.length > 950) {
				msg = `${msg.slice(0, 947)}...`;
			}
	...
	async function processPhase(state: GameState): Promise<void> {
		...
		try {
			...
			// Track NMRs — assigned powers that didn't submit orders
			const assignedPowersForNmr = state.players
				.map((p) => p.power)
				.filter((p): p is Power => p !== null);
			const mustOrderForNmr = state.orderablePowers
				? assignedPowersForNmr.filter((p) => (state.orderablePowers as Power[]).includes(p))
				: assignedPowersForNmr;
			const nmrPowers = mustOrderForNmr.filter((p) => !ordersMap[p]);
			const orderedPowers = mustOrderForNmr.filter((p) => ordersMap[p]);

			// Update consecutive NMR counts
			for (const power of orderedPowers) {
				db.setBotState(`nmr_${state.gameId}_${power}`, '0');
			}
			for (const power of nmrPowers) {
				const key = `nmr_${state.gameId}_${power}`;
				const prev = Number(db.getBotState(key) ?? '0');
				const count = prev + 1;
				db.setBotState(key, String(count));

				if (count >= 2) {
					// Alert game creator about chronic NMR
					const creator = state.players[0];
					if (creator) {
						const player = state.players.find((p) => p.power === power);
						const handle = player ? `@${player.handle}` : power;
						try {
							await dmSender.sendDm(
								creator.did,
								`⚠️ NMR alert for #${state.gameId}: ${handle} (${power}) has missed ${count} consecutive phases. Consider finding a replacement — anyone can claim an open power with "claim #${state.gameId} ${power}".`,
							);
						} catch (error) {
							console.warn(`[nmr] Failed to DM creator about NMR: ${error}`);
						}
					}
				}
			}
			...
			// Post orders map (with arrows/symbols) as a reply before text orders
			if (adjResult.ordersSvg) {
				try {
					const ordersMapPost = await postWithMapSvg(
						agent,
						`Orders — ${seasonName} ${phase.year}`,
						adjResult.ordersSvg,
						`Diplomacy orders map — ${seasonName} ${phase.year} (arrows show moves, lines show supports)`,
						{
							parentUri: orderReplyParent.uri,
							parentCid: orderReplyParent.cid,
							rootUri: phasePost.uri,
							rootCid: phasePost.cid,
						},
					);
					recordAndLabel(
						ordersMapPost.uri,
					...
					orderReplyParent = ordersMapPost;
				} catch (error) {
					console.warn(`[phase] Failed to post orders map for #${state.gameId}: ${error}`);
				}
			}
			...
			// Skip eliminated players (0 units, 0 centers) to avoid spam
			for (const player of advanced.players) {
				if (player.power) {
					const units = adjResult.units[player.power] ?? [];
					const centers = adjResult.centers[player.power] ?? [];
					if (units.length === 0 && centers.length === 0) continue;
					const unitList = `Your units: ${units.join(', ')}`;
					try {
						await dmSender.sendDm(
							player.did,
	...
	async function maybePostStatusUpdate(state: GameState): Promise<void> {
		...
		const mustOrder = state.orderablePowers
			...
			: assignedPowers;
		const totalNeeded = mustOrder.length;
		const ordersIn = totalNeeded - pending.length;

		// Don't post if all orders are already in (grace period handles that)
		if (ordersIn >= totalNeeded) return;

		const timeLeft = formatRelativeDeadline(state.phaseDeadline as string);

		const statusMsg = `📊 Game #${state.gameId} — ${state.currentPhase}\n\n${ordersIn}/${totalNeeded} orders in. ${timeLeft}.`;

		try {
			const prevPost = db.getLatestMapPost(state.gameId);
			...
			console.log(`[status] Posted update for #${state.gameId}: ${ordersIn}/${totalNeeded} orders`);
		} catch (error) {
			console.warn(`[status] Failed to post update for #${state.gameId}: ${error}`);
		}
	}

	/** DM deadline reminders at 12h, 3h, 1h remaining to players who haven't submitted orders.
	 *  Uses a bot_state key per game to track which checkpoint was last sent. */
	async function maybeSendDeadlineReminders(state: GameState): Promise<void> {
		if (!state.phaseDeadline) return;
		...
		// Determine current checkpoint: 12h, 3h, 1h
		let checkpoint: string | null = null;
		if (msRemaining <= 1 * HOUR) checkpoint = '1h';
		else if (msRemaining <= 3 * HOUR) checkpoint = '3h';
		else if (msRemaining <= 12 * HOUR) checkpoint = '12h';
		else return;

		const stateKey = `reminder_${state.gameId}`;
		const lastCheckpoint = db.getBotState(stateKey);
		...
		for (const power of pending) {
			const player = state.players.find((p) => p.power === power);
			if (!player) continue;
			try {
				await dmSender.sendDm(
					player.did,
					`Reminder: ${timeLeft} for #${state.gameId} (${state.currentPhase}). You haven't submitted orders yet.\n\nDM "#${state.gameId} possible" to see your options.`,
				);
			} catch (error) {
				console.warn(`[reminder] Failed to DM ${player.handle}: ${error}`);
			}
		}

		db.setBotState(stateKey, checkpoint);
		console.log(
			`[reminder] Sent ${checkpoint} reminders for #${state.gameId} to ${pending.length} players`,
		);
	}

	/** Tick — check deadlines on all active games, process expired ones, check queue */
	async function tick(): Promise<void> {
		// Check ready check expiry
		try {
			const readyDeadline = db.getBotState('ready_check_deadline');
			if (readyDeadline && new Date(readyDeadline) <= new Date()) {
				// Check if all confirmed first (edge case: last player confirmed right at deadline)
				if (db.allConfirmed(config.maxPlayers)) {
					console.log('[queue] All players confirmed at deadline, starting game');
					const noopReply = async (text: string) => {
						console.log(`[queue] matchmake: ${text}`);
					};
					await completeMatchmaking(noopReply);
				} else {
					await expireReadyCheck();
				}
			}
		} catch (error) {
			console.error('[queue] Error in tick ready check:', error);
		}

		// Safety-net: if queue is full and no ready check active, trigger one
		try {
			const readyCheckActive = db.getBotState('ready_check_deadline');
			if (!readyCheckActive && db.getQueueSize() >= config.maxPlayers) {
				const noopReply = async (text: string) => {
					console.log(`[queue] matchmake: ${text}`);
				};
				await matchmakeFromQueue(noopReply);
		...
		for (const state of activeGames) {
			...
			// Deadline reminder DMs (non-blocking)
			try {
				await maybeSendDeadlineReminders(state);
			} catch (error) {
				console.warn(`[reminder] Error checking reminders for #${state.gameId}: ${error}`);
			}
...
DM queries:
#id possible — See all unit options
#id possible F NTH — Options for one unit
#id orders — See submitted orders
#id map — Link to latest map
#id ready — Ready for immediate adjudication
#id draw — Vote for a draw
