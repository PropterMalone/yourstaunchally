...
def set_orders_and_process(game, orders, render=False):
    ...
    # Render BEFORE processing so order arrows/symbols are visible on the map.
    # After process() the orders are cleared and moved to history.
    pre_process_svg = None
    if render:
        centers_before = {name: game.get_centers(name) for name in game.get_map_power_names()}
        pre_process_svg = game.render(incl_orders=True, incl_abbrev=True)
        pre_process_svg = strip_non_sc_coloring(pre_process_svg, centers_before)
    ...
    if render:
        # Post-process map (new state, no orders) for the position view
        post_svg = game.render(incl_abbrev=True)
        result["svg"] = strip_non_sc_coloring(post_svg, result["centers"])
        # Pre-process map with order arrows
        result["orders_svg"] = pre_process_svg
...
def main():
    ...
    try:
        ...
        json.dump({"ok": False, "error": f"{type(e).__name__}: {e}", "traceback": tb}, sys.stdout)
